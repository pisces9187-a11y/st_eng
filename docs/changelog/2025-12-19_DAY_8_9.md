...
POST   /api/v1/production/recordings/upload/
       → Upload audio file (MP3, WAV, WebM, max 10MB)
       → Validate phoneme_id, audio_file, duration
       → Store file in user_recordings/%Y/%m/%d/
       → Auto-update UserPhonemeProgress
       → Return recording object

GET    /api/v1/production/recordings/
       → List user's recordings (paginated)
       → Filter by phoneme_id, is_best
       → Return stats (total, avg duration, avg score)

GET    /api/v1/production/recordings/<id>/
       → Get single recording details
       → Include phoneme full info

PATCH  /api/v1/production/recordings/<id>/update/
       → Update self_assessment_score (1-5 stars)
       → Update is_best (auto-unmark others)
...
GET    /api/v1/production/phonemes/<id>/recordings/
       → Get all recordings for specific phoneme
       → Return stats (total, avg score, has_best)
```

**2. Database Model** (Already implemented in Day 6-7)
- ✅ ProductionRecording model with file upload
- ✅ Auto file path: `user_recordings/%Y/%m/%d/recording.webm`
- ✅ Self-assessment score (1-5 stars)
- ✅ `is_best` flag (auto-unmarks previous best)
- ✅ File size tracking (bytes)
- ✅ Duration tracking (seconds)

**3. Admin Interface**
- ✅ ProductionRecordingAdmin already registered
- ✅ Star rating display (⭐⭐⭐⭐⭐)
- ✅ File size formatting (KB/MB)
- ✅ Best recording indicator

**4. URL Configuration**
- ✅ 6 API routes added to `apps/study/urls.py`
- ✅ 2 page routes added to `apps/curriculum/urls.py`
- ✅ All endpoints accessible under `/api/v1/production/`
...
- ✅ Stats display (total, unique phonemes, avg score)
- ✅ Filter by phoneme dropdown
...
- ✅ Rating modal for updating scores
...
- [ ] Click red record button → starts recording
...
- [ ] "Ghi lại" button discards and resets
...
- [ ] No microphone detected → shows error
- [ ] Microphone in use → handles gracefully
