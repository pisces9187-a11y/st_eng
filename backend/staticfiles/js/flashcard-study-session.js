...
const FlashcardStudySession = {
    ...
    async startSession(deckId = null, cardCount = null, level = null) {
        try {
            ...
            // Initialize session
            this.session.id = response.session_id;
            this.session.deckId = deckId;
            this.session.cards = response.cards || [];
            this.session.currentIndex = 0;
            this.session.startTime = new Date();
            this.session.dailyProgress = response.daily_progress || {};
            this.session.streak = response.streak || { current: 0, longest: 0 };
    ...
    async reviewCard(quality) {
        ...
        try {
            ...
            if (response.progress) {
                card.progress = response.progress;
                card.next_review = response.progress.next_review_at;
                card.is_mastered = response.progress.is_mastered;
            }

            // Check for achievements
            if (response.achievements_unlocked && response.achievements_unlocked.length > 0) {
                this.handleAchievementsUnlocked(response.achievements_unlocked);
            }

            // Check daily goal
            if (response.daily_progress) {
                this.session.dailyProgress = response.daily_progress;
                
                if (response.daily_progress.is_goal_reached && !this.session.goalReached) {
            ...
            if (this.callbacks.onReviewComplete) {
                this.callbacks.onReviewComplete(card, quality, response);
            }
