...
const FlashcardAudioPlayer = {
    ...
    async loadWord(word, options = {}) {
        ...
        if (audioUrl) {
            ...
            await new Promise((resolve, reject) => {
                const onCanPlay = () => {
                    this.elements.audioElement.removeEventListener('canplaythrough', onCanPlay);
                    this.elements.audioElement.removeEventListener('error', onError);
                    resolve();
                };
                ...
                const onError = (e) => {
                    this.elements.audioElement.removeEventListener('canplaythrough', onCanPlay);
                    this.elements.audioElement.removeEventListener('error', onError);
                    console.error('[FlashcardAudioPlayer] Error loading audio:', e);
                    reject(e);
                };
                
                this.elements.audioElement.addEventListener('canplaythrough', onCanPlay);
                this.elements.audioElement.addEventListener('error', onError);
                this.elements.audioElement.load();
            });

            console.log('[FlashcardAudioPlayer] Audio ready for:', word);
            ...
            console.warn('[FlashcardAudioPlayer] No audio URL for word:', word);
        }
    },
    ...
    setLoadingState(isLoading) {
        ...
        if (this.elements.loadingIndicator) {
            // Disable loading indicator to prevent flickering
