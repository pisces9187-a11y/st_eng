"""
Script to populate MinimalPair database with meaningful phoneme pairs.
Run from Django shell or directly.
"""
...
def populate():
    ...
    for pair_data in MINIMAL_PAIRS:
        try:
            # Get phonemes
            ...
            if existing:
                skipped += 1
                print(f"[SKIP] {pair_data['p1_sym']} vs {pair_data['p2_sym']}: {pair_data['w1']} vs {pair_data['w2']}")
                continue
            ...
            created += 1
            print(f"[OK] {pair_data['p1_sym']} vs {pair_data['p2_sym']}: {pair_data['w1']} vs {pair_data['w2']}")

        except Phoneme.DoesNotExist as e:
            skipped += 1
            print(f"[ERROR] Phoneme not found: {pair_data['p1_sym']} or {pair_data['p2_sym']}")
