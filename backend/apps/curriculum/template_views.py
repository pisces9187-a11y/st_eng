...
class PronunciationLibraryView(TemplateView):
    ...
    Shows all available IPA lessons organized by curriculum stages.
    """
    ...
    def get_context_data(self, **kwargs):
        ...
        # Get all curriculum stages with their lessons
        from .models import CurriculumStage
        stages = CurriculumStage.objects.filter(
            is_active=True
        ).prefetch_related(
            'lessons__phonemes',
            'lessons__stage'
        ...
        # Prepare stage data with progress
        stages_data = []
        total_lessons = 0
        completed_lessons = 0
        
        for stage in stages:
            lessons = stage.lessons.filter(status='published').order_by('part_number', 'unit_number', 'order')
            stage_lessons_count = lessons.count()
            total_lessons += stage_lessons_count
            ...
            for lesson in lessons:
                can_access, reason = lesson.can_access(user)
                progress = lesson.get_user_progress(user)
                
                lessons_with_access.append({
                    'lesson': lesson,
                    'can_access': can_access,
                    'lock_reason': reason if not can_access else None,
                    'progress': progress,
            ...
            stage_progress_percent = int((stage_completed / stage_lessons_count * 100)) if stage_lessons_count > 0 else 0
            ...
            stages_data.append({
                ...
                'is_unlocked': stage_completed > 0 or stage.number == 1,  # Stage 1 always unlocked
            })
        
        context['stages'] = stages_data
        context['total_lessons'] = total_lessons
        context['completed_lessons'] = completed_lessons
        context['overall_progress'] = int((completed_lessons / total_lessons * 100)) if total_lessons > 0 else 0
        
        # Legacy: Keep categories for any old code that might reference it
