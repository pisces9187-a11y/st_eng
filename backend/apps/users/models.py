...
#============================================================================
# TONGUE TWISTER CHALLENGE MODELS
#============================================================================

class TongueTwisterAttempt(models.Model):
    """
    Tracks user attempts at tongue twister challenges.
    Gamification feature for pronunciation practice.
    """
    user = models.ForeignKey(
        User,
        on_delete=models.CASCADE,
        related_name='tongue_twister_attempts',
        verbose_name='Người dùng'
    )
    tongue_twister = models.ForeignKey(
        'curriculum.TongueTwister',
        on_delete=models.CASCADE,
    ...
    audio_file = models.FileField(
        upload_to='tongue_twister_recordings/%Y/%m/',
    ...
    duration = models.FloatField(
        ...
        help_text='Time taken to complete the tongue twister'
    )
    score = models.FloatField(
        default=0.0,
        verbose_name='Điểm (0-100)',
        help_text='Overall performance score'
    )
    
    # Speech recognition results
    words_detected = models.PositiveIntegerField(
        default=0,
        verbose_name='Số từ phát hiện được'
    )
    words_expected = models.PositiveIntegerField(
        default=0,
        verbose_name='Số từ dự kiến'
    )
    accuracy = models.FloatField(
        default=0.0,
        verbose_name='Độ chính xác (%)',
        help_text='Percentage of correctly pronounced words'
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=['tongue_twister', '-score']),
        ]
    
    def __str__(self):
        return f"{self.user.username} - {self.tongue_twister.text[:30]} - {self.score}pts"
    
    @property
    def words_per_second(self):
        """Calculate speaking speed"""
        if self.duration > 0:
            return self.words_detected / self.duration
        return 0
    
    @property
    def completion_percentage(self):
        """Percentage of expected words detected"""
        if self.words_expected > 0:
            return (self.words_detected / self.words_expected) * 100
        return 0
