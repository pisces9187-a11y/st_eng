...
class HomePageView(TemplateView):
    ...
    If user is already authenticated via JWT, redirect to dashboard.
    """
    template_name = 'public/home.html'
    
    def dispatch(self, request, *args, **kwargs):
        # Only redirect if user has valid JWT tokens
        has_jwt_token = (
        ...
        if getattr(request, 'jwt_authenticated', False) and has_jwt_token:
...
class LoginPageView(TemplateView):
    ...
    If user is already authenticated via JWT, redirect to dashboard.
    IMPORTANT: Only redirect if JWT tokens exist, not just Django session.
    """
    template_name = 'users/login.html'
    
    def dispatch(self, request, *args, **kwargs):
        # Only redirect if user has valid JWT tokens
        # Check for JWT authentication AND token presence to avoid redirect loop
        has_jwt_token = (
            request.COOKIES.get('access_token') or 
            request.COOKIES.get('refresh_token') or
            request.headers.get('Authorization', '').startswith('Bearer ')
