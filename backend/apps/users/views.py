...
class LogoutView(APIView):
    ...
    def post(self, request):
        try:
            ...
            # CRITICAL: Flush Django session completely to prevent redirect loop
            # This ensures the session is fully cleared, not just logged out
            ...
            response.delete_cookie('sessionid', path='/', samesite='Lax')  # Django session
            response.delete_cookie('csrftoken', path='/', samesite='Lax')  # CSRF token
