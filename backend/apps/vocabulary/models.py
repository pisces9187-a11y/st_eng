...
# Import achievement models to register them
from .models_achievement import Achievement, UserAchievement, check_and_unlock_achievements

# Import study tracking models to register them
...
class StudySession(models.Model):
    ...
    Enhanced with streak tracking and daily goals.
    """
    ...
    # NEW: Streak & Goals tracking
    streak_count = models.IntegerField(
        default=0,
        help_text="User's streak at time of this session"
    )
    daily_goal = models.IntegerField(
        default=20,
        help_text="User's daily goal at time of session"
    )
    cards_goal_today = models.IntegerField(
        default=0,
        help_text="Total cards learned today (including this session)"
    )
    is_goal_reached = models.BooleanField(
        default=False,
        help_text="Whether daily goal was reached in this session"
    )
